...
*&
*&---------------------------------------------------------------------*
REPORT ZBCLL_COSTING_FG_CCPC_A1.
TABLES : ZTP_COST5,
         MARA,
         MAKT,
         LFA1.
...
DATA: IT_ZTP_COST5 TYPE TABLE OF ZTP_COST5,
...
DATA: ZTP_COST5_WA TYPE ZTP_COST5.
***************************************************************************************
...
*----------------------------------------------------------------------*
FORM DISPLAY .
  SELECT * FROM ZTP_COST5 INTO TABLE IT_ZTP_COST5 WHERE MATNR IN MATERIAL.
  IF SY-SUBRC NE 0.
    MESSAGE 'NO DATA FOUND' TYPE 'E'.
  ENDIF.
  LOOP AT IT_ZTP_COST5 INTO WA_ZTP_COST5.
    WA_TAB1-MATNR = WA_ZTP_COST5-MATNR.
    WA_TAB1-LIFNR = WA_ZTP_COST5-LIFNR.
    SELECT SINGLE * FROM LFA1 WHERE LIFNR EQ WA_ZTP_COST5-LIFNR.
    IF SY-SUBRC EQ 0.
      WA_TAB1-NAME1 = LFA1-NAME1.
    ENDIF.
    SELECT SINGLE * FROM MAKT WHERE MATNR EQ WA_ZTP_COST5-MATNR AND SPRAS EQ 'EN'.
    IF SY-SUBRC EQ 0.
      WA_TAB1-MAKTX = MAKT-MAKTX.
    ENDIF.
    WA_TAB1-BUDAT = WA_ZTP_COST5-BUDAT.
    WA_TAB1-CC = WA_ZTP_COST5-CC.
    WA_TAB1-PC = WA_ZTP_COST5-PC.
    WA_TAB1-SGTXT = WA_ZTP_COST5-SGTXT.
    WA_TAB1-CPUDT = WA_ZTP_COST5-CPUDT.
    WA_TAB1-UZEIT = WA_ZTP_COST5-UZEIT.
  ...
  LAYOUT-WINDOW_TITLEBAR  = 'FINISHED PRODUCT, VENDOR WISE CCPC RATES FOR THIRD PARTY COSTING'.
...
FORM TOP.
  ...
  WA_COMMENT-TYP = 'A'.
  WA_COMMENT-INFO = 'FINISHED PRODUCT VENDOR WISE CC & PC RATES'.
...
*  LV_FLDCAT-TABNAME   = 'IT_TAB1'.
  ...
  LV_FLDCAT-FIELDNAME = 'CC'.
  LV_FLDCAT-SCRTEXT_M = 'CC RATE'.
  LV_FLDCAT-EDIT   = 'X'.
  LV_FLDCAT-OUTPUTLEN = 10.
  APPEND LV_FLDCAT TO IT_FCAT.
  CLEAR LV_FLDCAT.

  LV_FLDCAT-FIELDNAME = 'PC'.
  LV_FLDCAT-SCRTEXT_M = 'PC RATE'.
  LV_FLDCAT-EDIT   = 'X'.
  LV_FLDCAT-OUTPUTLEN = 10.
  APPEND LV_FLDCAT TO IT_FCAT.
  CLEAR LV_FLDCAT.
...
*    SELECT SINGLE * FROM MARA WHERE MATNR EQ WA_TAB1-MATNR AND MTART IN ( 'ZHWA','ZDSM' ).
  ...
  LOOP AT IT_TAB1 INTO WA_TAB1 WHERE MATNR GT 0.
    ZTP_COST5_WA-MATNR = WA_TAB1-MATNR.
    ZTP_COST5_WA-LIFNR = WA_TAB1-LIFNR.
    ZTP_COST5_WA-BUDAT = WA_TAB1-BUDAT.
    ZTP_COST5_WA-CC = WA_TAB1-CC.
    ZTP_COST5_WA-PC = WA_TAB1-PC.
    ZTP_COST5_WA-SGTXT = WA_TAB1-SGTXT.

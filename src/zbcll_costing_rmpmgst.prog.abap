...
*&
*&---------------------------------------------------------------------*
REPORT ZBCLL_COSTING_RMPMGST_A1.
TABLES : ZTP_COST2,
         MARA,
         MAKT.
...
DATA: IT_ZTP_COST2 TYPE TABLE OF ZTP_COST2,
...
DATA: ZTP_COST2_WA TYPE ZTP_COST2.
***************************************************************************************
...
*----------------------------------------------------------------------*
FORM DISPLAY .
  SELECT * FROM ZTP_COST2 INTO TABLE IT_ZTP_COST2 WHERE MATNR IN MATERIAL.
  IF SY-SUBRC NE 0.
    MESSAGE 'NO DATA FOUND' TYPE 'E'.
  ENDIF.
  LOOP AT IT_ZTP_COST2 INTO WA_ZTP_COST2.
    WA_TAB1-MATNR = WA_ZTP_COST2-MATNR.
    SELECT SINGLE * FROM MAKT WHERE MATNR EQ WA_ZTP_COST2-MATNR AND SPRAS EQ 'EN'.
    IF SY-SUBRC EQ 0.
      WA_TAB1-MAKTX = MAKT-MAKTX.
    ENDIF.
    WA_TAB1-BUDAT = WA_ZTP_COST2-BUDAT.
    WA_TAB1-PERCNT = WA_ZTP_COST2-PERCNT.
    WA_TAB1-PERCNT = WA_ZTP_COST2-PERCNT.
    WA_TAB1-SGTXT = WA_ZTP_COST2-SGTXT.
    WA_TAB1-CPUDT = WA_ZTP_COST2-CPUDT.
    WA_TAB1-UZEIT = WA_ZTP_COST2-UZEIT.
  ...
  WA_FIELDCAT-SELTEXT_L = 'GST RATE EFFECTIVE DATE'.
  APPEND WA_FIELDCAT TO FIELDCAT.
  ...
  LAYOUT-WINDOW_TITLEBAR  = 'MATERIAL GST RATES FOR THIRD PARTY COSTING'.
...
FORM TOP.
  ...
  WA_COMMENT-TYP = 'A'.
  WA_COMMENT-INFO = 'MATERIAL GST RATE DETAILS'.
...
*  LV_FLDCAT-TABNAME   = 'IT_TAB1'.
  ...
  LV_FLDCAT-SCRTEXT_M = 'GST RATE EFFECTIVE FROM'.
  LV_FLDCAT-EDIT   = 'X'.
  LV_FLDCAT-OUTPUTLEN = 10.
  APPEND LV_FLDCAT TO IT_FCAT.
  CLEAR LV_FLDCAT.
...
FORM DATAUPD .
  ...
  LOOP AT IT_TAB1 INTO WA_TAB1 WHERE PERCNT GT 0.
    ZTP_COST2_WA-MATNR = WA_TAB1-MATNR.
    ZTP_COST2_WA-BUDAT = WA_TAB1-BUDAT.
    ZTP_COST2_WA-PERCNT = WA_TAB1-PERCNT.
    ZTP_COST2_WA-SGTXT = WA_TAB1-SGTXT.

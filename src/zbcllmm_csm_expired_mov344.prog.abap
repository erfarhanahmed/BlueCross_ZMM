...
data : it_tab1 type table of itab1,
       ...
       wa_tab2 type itab1,
       it_mov1 type table of mov1,
       wa_mov1 type mov1,
       it_mov2 type table of mov2,
       wa_mov2 type mov2.
...
select-options : pbudat for mkpf-budat.
...
start-of-selection.
  ...
  if r1 eq 'X' or r2 eq 'X'.
    if pdate is initial.
      message 'ENTER DUE DATE FOR DESTRUCTION' type 'E'.
    endif.
  endif.

  if r3 eq 'X'.
    if pbudat is initial.
...
*      WRITE:  date1.
          if date1 ge pdate-low and date1 le pdate-high.
            ...
            wa_tab1-date1 = date1.
****************mov
            select single * from mseg where bwart eq '344' and matnr eq wa_mchb-matnr and charg eq wa_mchb-charg and menge eq wa_mchb-cspem and lgort eq 'CSM'.
            if sy-subrc eq 0.
              wa_tab1-mblnr = mseg-mblnr.
              select single * from mkpf where mjahr eq mseg-mjahr and mblnr eq mseg-mblnr.
              if sy-subrc eq 0.
...
form top.
  ...
  elseif r3 eq 'X'.
    wa_comment-info = 'POSTED DOCUMENT DETAILS'.
  endif.
...
IF PLANT EQ '1000'.
  FORMAT = 'Format No.: SOP/QAD/074-06-F1(Ref. SOP No.: SOP/QAD/074)'.
ENDIF.
...
form alvdisp .
  ...
  wa_fieldcat-seltext_l = '344 DOC NO.'.
  append wa_fieldcat to fieldcat.

  wa_fieldcat-fieldname = 'BUDAT'.
  wa_fieldcat-seltext_l = '344 MOV DT'.
  append wa_fieldcat to fieldcat.
...
*GOODSMVT_ITEM-MVT_IND    = '' .
    ...
    goodsmvt_item-gl_account = '0000004030'.
...
*----------------------------------------------------------------------*
form postdetails .
  select * from mkpf into table it_mkpf where budat in pbudat.
  if sy-subrc eq 0.
    select * from mseg into table it_mseg for all entries in it_mkpf where mblnr eq it_mkpf-mblnr and mjahr eq it_mkpf-mjahr and bwart eq '555'
      and lgort eq 'CSM' and werks eq plant.
  endif.
  ...
  if it_mseg is not initial.
    loop at it_mseg into wa_mseg.
      ...
      if sy-subrc eq 0.
*        write : / wa_mkpf-budat,wa_mseg-mblnr,wa_mseg-mjahr,wa_mseg-matnr,wa_mseg-charg,wa_mseg-menge,wa_mseg-bwart,wa_mseg-lgort.
        wa_mov1-budat = wa_mkpf-budat.
        wa_mov1-mblnr = wa_mseg-mblnr.
        wa_mov1-mjahr = wa_mseg-mjahr.
        wa_mov1-werks = wa_mseg-werks.
        wa_mov1-matnr = wa_mseg-matnr.
        wa_mov1-charg = wa_mseg-charg.
        wa_mov1-menge = wa_mseg-menge.
        wa_mov1-bwart = wa_mseg-bwart.
        wa_mov1-lgort = wa_mseg-lgort.
        wa_mov1-dmbtr = wa_mseg-dmbtr.
        collect wa_mov1 into it_mov1.
        clear wa_mov1.
      endif.
    endloop.
  endif.
  ...
  loop at it_mov1 into wa_mov1.
    write : / wa_mov1-budat,wa_mov1-mblnr,wa_mov1-mjahr,wa_mov1-matnr,wa_mov1-charg,wa_mov1-menge,wa_mov1-bwart,wa_mov1-lgort.
    wa_mov2-budat = wa_mov1-budat.
    wa_mov2-mblnr = wa_mov1-mblnr.
    wa_mov2-mjahr = wa_mov1-mjahr.
    wa_mov2-matnr = wa_mov1-matnr.
    wa_mov2-charg = wa_mov1-charg.
    wa_mov2-menge = wa_mov1-menge.
    wa_mov2-bwart = wa_mov1-bwart.
    wa_mov2-lgort = wa_mov1-lgort.
    wa_mov2-dmbtr = wa_mov1-dmbtr.

    select single * from mcha where matnr eq wa_mov1-matnr and werks eq wa_mov1-werks and charg eq wa_mov1-charg and vfdat ne 0.
    if sy-subrc eq 0.
      write : mcha-hsdat,mcha-vfdat.
      wa_mov2-hsdat = mcha-hsdat.
      wa_mov2-vfdat = mcha-vfdat.
    endif.
    select single * from makt where matnr eq wa_mov1-matnr and spras eq 'EN'.
    if sy-subrc eq 0.
      write : makt-maktx.
      wa_mov2-maktx = makt-maktx.
    endif.
    select single * from mseg where bwart eq '344' and matnr eq wa_mov1-matnr and charg eq wa_mov1-charg and menge eq wa_mov1-menge and lgort eq 'CSM'.
    if sy-subrc eq 0.
      write : / mseg-mblnr.
      wa_mov2-mblnr344 = mseg-mblnr.
      select single * from mkpf where mjahr eq mseg-mjahr and mblnr eq mseg-mblnr.
      if sy-subrc eq 0.
        write : mkpf-budat.
        wa_mov2-budat344 = mkpf-budat.
      endif.
    endif.
    collect wa_mov2 into it_mov2.
    clear wa_mov2.
  endloop.

*  loop at it_mov2 into wa_mov2.
*    write : / wa_mov2-budat,wa_mov2-mblnr,wa_mov2-mjahr,wa_mov2-matnr,wa_mov2-charg,wa_mov2-menge,wa_mov2-bwart,wa_mov2-lgort.
*    write: wa_mov2-hsdat,wa_mov2-vfdat,wa_mov2-maktx,wa_mov2-mblnr344,wa_mov2-budat344.
*  endloop.
  ...
  wa_fieldcat-fieldname = 'MBLNR'.
  wa_fieldcat-seltext_l = 'POSTING DOC'.
  append wa_fieldcat to fieldcat.
  ...
  wa_fieldcat-fieldname = 'MBLNR344'.
  wa_fieldcat-seltext_l = '344 DOC NO.'.
  append wa_fieldcat to fieldcat.

  wa_fieldcat-fieldname = 'BUDAT344'.
  wa_fieldcat-seltext_l = '344 MOV DT'.
  append wa_fieldcat to fieldcat.
  ...
  layout-zebra = 'X'.
  layout-colwidth_optimize = 'X'.
  layout-window_titlebar  = '555 CSM POSTED DOCUMENTS'.
  layout-info_fieldname  = 'LINE_COLOR'.

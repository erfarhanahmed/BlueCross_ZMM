...
*&
*&---------------------------------------------------------------------*
REPORT ZBCLL_COSTING_FG_PROFIT_A1.
TABLES : ZTP_COST4,
         MARA,
         MAKT,
         LFA1.
...
DATA: IT_ZTP_COST4 TYPE TABLE OF ZTP_COST4,
...
DATA: ZTP_COST4_WA TYPE ZTP_COST4.
***************************************************************************************
...
*----------------------------------------------------------------------*
FORM DISPLAY .
  SELECT * FROM ZTP_COST4 INTO TABLE IT_ZTP_COST4 WHERE MATNR IN MATERIAL.
  IF SY-SUBRC NE 0.
    MESSAGE 'NO DATA FOUND' TYPE 'E'.
  ENDIF.
  LOOP AT IT_ZTP_COST4 INTO WA_ZTP_COST4.
    WA_TAB1-MATNR = WA_ZTP_COST4-MATNR.
    WA_TAB1-LIFNR = WA_ZTP_COST4-LIFNR.
    SELECT SINGLE * FROM LFA1 WHERE LIFNR EQ WA_ZTP_COST4-LIFNR.
    IF SY-SUBRC EQ 0.
      WA_TAB1-NAME1 = LFA1-NAME1.
    ENDIF.
    SELECT SINGLE * FROM MAKT WHERE MATNR EQ WA_ZTP_COST4-MATNR AND SPRAS EQ 'EN'.
    IF SY-SUBRC EQ 0.
      WA_TAB1-MAKTX = MAKT-MAKTX.
    ENDIF.
    WA_TAB1-BUDAT = WA_ZTP_COST4-BUDAT.
    WA_TAB1-PERCNT = WA_ZTP_COST4-PERCNT.
    WA_TAB1-SGTXT = WA_ZTP_COST4-SGTXT.
    WA_TAB1-CPUDT = WA_ZTP_COST4-CPUDT.
    WA_TAB1-UZEIT = WA_ZTP_COST4-UZEIT.
  ...
  WA_FIELDCAT-SELTEXT_L = 'PROFIT PRECENT'.
  APPEND WA_FIELDCAT TO FIELDCAT.
  ...
  LAYOUT-WINDOW_TITLEBAR  = 'FINISHED PRODUCT, VENDOR WISE PROFIT PERCENT FOR THIRD PARTY COSTING'.
...
FORM TOP.
  ...
  WA_COMMENT-TYP = 'A'.
  WA_COMMENT-INFO = 'FINISHED PRODUCT VENDOR WISE PROFIT PERCENT'.
...
*  LV_FLDCAT-TABNAME   = 'IT_TAB1'.
  ...
  LV_FLDCAT-SCRTEXT_M = 'PROFIT PERCENT'.
  LV_FLDCAT-EDIT   = 'X'.
  LV_FLDCAT-OUTPUTLEN = 10.
  APPEND LV_FLDCAT TO IT_FCAT.
  CLEAR LV_FLDCAT.
...
*    SELECT SINGLE * FROM MARA WHERE MATNR EQ WA_TAB1-MATNR AND MTART in ( 'ZHWA','ZDSM' ).
  ...
  LOOP AT IT_TAB1 INTO WA_TAB1 WHERE PERCNT GT 0.
    ZTP_COST4_WA-MATNR = WA_TAB1-MATNR.
    ZTP_COST4_WA-LIFNR = WA_TAB1-LIFNR.
    ZTP_COST4_WA-BUDAT = WA_TAB1-BUDAT.
    ZTP_COST4_WA-PERCNT = WA_TAB1-PERCNT.
    ZTP_COST4_WA-SGTXT = WA_TAB1-SGTXT.

...
*&
*&---------------------------------------------------------------------*
report zbcll_costing_fg_yield_a1.
tables : ztp_cost7,
         mara,
         makt,
         lfa1.
...
data: it_ztp_cost7 type table of ztp_cost7,
...
data: ztp_cost7_wa type ztp_cost7.
***************************************************************************************
...
*----------------------------------------------------------------------*
form display .
  select * from ztp_cost7 into table it_ztp_cost7 where matnr in material.
  if sy-subrc ne 0.
    message 'NO DATA FOUND' type 'E'.
  endif.
  loop at it_ztp_cost7 into wa_ztp_cost7.
    wa_tab1-matnr = wa_ztp_cost7-matnr.
    wa_tab1-lifnr = wa_ztp_cost7-lifnr.
    select single * from lfa1 where lifnr eq wa_ztp_cost7-lifnr.
    if sy-subrc eq 0.
      wa_tab1-name1 = lfa1-name1.
    endif.
    select single * from makt where matnr eq wa_ztp_cost7-matnr and spras eq 'EN'.
    if sy-subrc eq 0.
      wa_tab1-maktx = makt-maktx.
    endif.
    wa_tab1-budat = wa_ztp_cost7-budat.
    wa_tab1-percnt = wa_ztp_cost7-percnt.
    wa_tab1-PMpercnt = wa_ztp_cost7-PMpercnt.
    wa_tab1-sgtxt = wa_ztp_cost7-sgtxt.
    wa_tab1-uname = wa_ztp_cost7-uname.
    wa_tab1-cpudt = wa_ztp_cost7-cpudt.
    wa_tab1-uzeit = wa_ztp_cost7-uzeit.
  ...
  wa_fieldcat-seltext_l = 'YIELD %'.
  append wa_fieldcat to fieldcat.

  wa_fieldcat-fieldname = 'PMPERCNT'.
  wa_fieldcat-seltext_l = 'PM YIELD %'.
  append wa_fieldcat to fieldcat.
  ...
  layout-window_titlebar  = 'FINISHED PRODUCT, VENDOR WISE YIELD % FOR THIRD PARTY COSTING'.
...
form top.
  ...
  wa_comment-typ = 'A'.
  wa_comment-info = 'FINISHED PRODUCT VENDOR WISE YIELD %'.
...
*  LV_FLDCAT-TABNAME   = 'IT_TAB1'.
  ...
  lv_fldcat-scrtext_m = 'YIELD %'.
  lv_fldcat-edit   = 'X'.
  lv_fldcat-outputlen = 10.
  append lv_fldcat to it_fcat.
  clear lv_fldcat.

  lv_fldcat-fieldname = 'PMPERCNT'.
  lv_fldcat-scrtext_m = 'PM YIELD %'.
  lv_fldcat-edit   = 'X'.
  lv_fldcat-outputlen = 10.
  append lv_fldcat to it_fcat.
  clear lv_fldcat.
...
*    select single * from mara where matnr eq wa_tab1-matnr and mtart in ( 'ZHWA', 'ZDSM' ).
  ...
  loop at it_tab1 into wa_tab1 where percnt gt 0.
    ztp_cost7_wa-matnr = wa_tab1-matnr.
    ztp_cost7_wa-lifnr = wa_tab1-lifnr.
    ztp_cost7_wa-budat = wa_tab1-budat.
    ztp_cost7_wa-percnt = wa_tab1-percnt.
     ztp_cost7_wa-PMpercnt = wa_tab1-PMpercnt.
    ztp_cost7_wa-sgtxt = wa_tab1-sgtxt.
